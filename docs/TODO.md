...
The detailed experiment log for the Brix-Ruiz sidecar work now lives in [brix-ruiz-sidecar-log.md](brix-ruiz-sidecar-log.md).

Current takeaway:

- the sidecar evidence is now consistent across several structured move families: balanced witnesses, out-split refinements, mixed out/in refinements, and bounded `3x3 -> 2x2 -> 3x3` zig-zags all stay negative on the current bounds
- that makes wider split-sidecar expansion lower priority than changes to the main solver
- the next mainline search iteration should use this structure as guidance for search ordering or iterative deepening, rather than plain FIFO BFS
